                            ...
                            <div class="tm:w-1/2 px-4 py-6 bg-gray-100 dark:bg-gray-800">
                                <form action="{{ route('tracks.update', ['id' => $track->id]) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <input type="file" name="image" id="image" aria-describedby="file_input_help" class="block w-full text-sm file-btn text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="name" name="name" value="{{ $track->name }}" class="input dark:text-black">
                                    </div>
                                    <div class="mt-4">
                                        <label for="description" class="block font-semibold text-gray-900 dark:text-white">Description:</label>
                                        <textarea id="description" name="description" class="input dark:text-black">{{ $track->description }}</textarea>
                                    </div>
                                    <div class="mt-4">
                                        <label for="price_per_hour" class="block font-semibold text-gray-900 dark:text-white">Price per Hour:</label>
                                        <input type="number" id="price_per_hour" name="price_per_hour" value="{{ $track->price_per_hour }}" class="input dark:text-black">
                                    </div>
                                    <!-- Add other fields here -->
                                    {{-- <div class="mt-4">
                                        <label for="is_available" class="block font-semibold text-gray-900 dark:text-white">Availability:</label>
                                        <div class="flex items-center">
                                            <input type="checkbox" id="is_available" name="is_available" class="mr-2">
                                            <label for="is_available" class="text-gray-700 dark:text-gray-400">Available</label>
                                        </div>
                                    </div> --}}
                                    <div class="mt-4">
                                        <label for="is_available" class="block font-semibold text-gray-900 dark:text-white">Availability:</label>
